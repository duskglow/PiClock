...
from PyQt5.QtGui import QTransform, QKeyEvent
...
def wxfinished_owm_current():
    ...
    if not supress_current:
        ...
        icon = owmicons[icon]
        wxiconpixmap = QtWidgets.QPixmap(Config.icons + "/" + icon + ".png")
...
def wxfinished_owm_forecast():
    ...
    for i in range(0, 3):
        ...
        wxiconpixmap = QtWidgets.QPixmap(
...
def wxfinished_owm():
    ...
    for i in range(3, 9):
        ...
        fl = forecast[i]
        icon = fl.findChild(QtWidgets.QLabel, "icon")
        wxiconpixmap = QtWidgets.QPixmap(Config.icons + "/" + wicon + ".png")
        icon.setPixmap(wxiconpixmap.scaled(
            icon.width(),
            icon.height(),
            Qt.IgnoreAspectRatio,
...
class SS(QtWidgets.QLabel):
    ...
    def show_image(self, image):
        ...
        bg = QtWidgets.QPixmap.fromImage(image)
...
class Radar(QtWidgets.QLabel):
    ...
    def getbase(self):
        ...
        #QObject.connect(self.basereply, QtCore.SIGNAL(
...
class myMain(QtWidgets.QWidget):
    ...
    def mousePressEvent(self, event):
        if type(event) == QtWidgets.QMouseEvent:
            nextframe(1)
...
w.setStyleSheet("QtWidget { background-color: black;}")

# fullbgpixmap = QtWidgets.QPixmap(Config.background)
...
for i in range(0, 9):
    ...
    lab.setStyleSheet("QtWidget { background-color: transparent; color: " +
                      Config.textcolor +
                      "; font-size: " +
                      str(int(20 * xscale * Config.fontmult)) +
                      "px; " +
